...
public class processNAF {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 0) {
            ...
            System.err
                    .println(
                            "eu.fbk.knowledgestore.populator.naf.processNAF path disabled_items \ndisabled_items = [Entities|Mentions|Resources] ");
            throw new Exception();
            // System.exit(-1);
        }
    ...
    public static statistics readFile(String filepath, String disabled_Items, processNAFVariables vars)
            throws Exception {
        ...
        if (disabled_Items != null
                && (disabled_Items.matches("(?i)Entity") || disabled_Items.contains("(?i)Mention") || disabled_Items
                .contains("(?i)Resource"))) {
	    ...
	    */
		    ...
		    */
                    ...
                    for (Span spansObj : ((References) generalEntObj).getSpan()) {
                        ...
                        charS = m.getUnique(NIF.BEGIN_INDEX, Integer.class) + "," + m
                                .getUnique(NIF.END_INDEX, Integer.class);
			...
			*/
                        ...
                        String type3charLC = "";
                        if (!dbpedia) {
                            if (entObj.getType() != null && entObj.getType() != "" && !entObj.getType()
                                    .equalsIgnoreCase("misc")) {
                            ...
                            if (keepEntityTypeProvidedByNaf) {
                                URI dynamicTypeUri = ValueFactoryImpl.getInstance()
                                        .createURI(NWR.NAMESPACE, "entity_type_" + entObj.getType().toLowerCase());
                                ...
                                if (vars.entityTypeMapper.containsKey(type3charLC)
                                    ...
                                    logDebug("ROL1: <entity> STRANGE added new mention for id " + entObj.getId()
                                            + ", charSpan |"
                                            + getCharSpanFromSpan(spansObj, vars) + "|, type " + vars.entityTypeMapper
                                            .get(type3charLC), vars);
                                } else {
                                    addMentionFlag = false;
                            ...
                            if (entObj.getType() == null || entObj.getType().equals("")) {
                                ...
                                if (entObj.getType().toLowerCase().contains("per") || entObj.getType().toLowerCase()
                                        .contains("dbpedia:person")) {
                                    entObj.setType("person");
                                    type3charLC = entObj.getType().substring(0, 3).toLowerCase();
                                } else if (entObj.getType().toLowerCase().contains("org") || entObj.getType()
                                        .toLowerCase().contains("dbpedia:organisation")) {
                                    entObj.setType("organization");
                                    type3charLC = entObj.getType().substring(0, 3).toLowerCase();
                                } else if (entObj.getType().toLowerCase().contains("loc") || entObj.getType()
                                        .toLowerCase().contains("DBpedia:Place")) {
                        ...
                        if (addMentionFlag) {
                            if (addOrMergeAMention(m, vars) == 1) {
                                String charS2 = m.getUnique(NIF.BEGIN_INDEX, Integer.class) + "," + m
                                        .getUnique(NIF.END_INDEX, Integer.class);
		    ...
		    */
                    ...
                    List<ExternalRef> externalRefs = ((ExternalReferences) generalEntObj).getExternalRef();
//                    for (ExternalRef externalRef : externalRefs) {
//                        System.out.println(externalRef);
//                        System.out.println(externalRef.getReference());
//                        System.out.println(externalRef.getSource());
//                        System.out.println(externalRef.getResource());
//                        System.out.println();
//                    }
//
                    ...
                    if (!disabledItems.matches("(?i)Entity")) {
                        ...
                        for (ExternalRef exRObj : externalRefs) {
                            if (exRObj.getSource() == null) {
                                exRObj.setSource("en");
                            }
                            ...
                            if (exRObj.getSource().equalsIgnoreCase("POCUS")) {
                                if (referencesElements < 1) {
                                    logDebug(
                                            "Every entity must contain a 'references' element:not possible to add ExternalRef to null.",
                                            vars);
                                    continue;
                                }
                        ...
                        if (modeactive) {
                            ...
                            for (ExternalRef exRObj : externalRefs) {
                                if (exRObj != null) {
                                    getAllLayersOfExternalReferences(modeactive, exrEn, exRObj, exrEs, exrIt, exrNl,
                                            vars);
                                }
                            }

                            String highConfidenceReferenceValue = null;
                            if (exrEn.size() > 0) {
                                highConfidenceReferenceValue = getHighConfidenceReferenceValue(exrEn);
                            } else if (exrEs.size() > 0) {
                                highConfidenceReferenceValue = getHighConfidenceReferenceValue(exrEs);
                            } else if (exrIt.size() > 0) {
                                highConfidenceReferenceValue = getHighConfidenceReferenceValue(exrIt);
                            } else if (exrNl.size() > 0) {
                                highConfidenceReferenceValue = getHighConfidenceReferenceValue(exrNl);
                            }

                            if (highConfidenceReferenceValue != null) {
                                chosenReferenceValue = new String(highConfidenceReferenceValue);
                            }
...
//                            }
    ...
    private static void getAllLayersOfExternalReferences(boolean modeactive, LinkedList<ExternalRef> exrEn,
            ExternalRef exRObj, LinkedList<ExternalRef> exrEs, LinkedList<ExternalRef> exrIt,
            LinkedList<ExternalRef> exrNl, processNAFVariables vars) {
        ...
        if (modeactive) {
            if (exRObj.getReftype() != null) {
                ...
                if (exRObj.getReference().contains("://dbpedia.org")) {
                    ...
                    exrIt.addLast(exRObj);
                } else if (exRObj.getReference().contains("://nl.dbpedia.org")) {
                    exrNl.addLast(exRObj);
                }
            } else {
                logDebug(
                        "Every entity must contain a 'references' element with type or DBpedia reference: not possible to add ExternalRef to null.",
                        vars);
            }
        }
    ...
    private static void getTimeExpressionsMentions(TimeExpressions obj, processNAFVariables vars) throws Exception {
        ...
        for (Timex3 tmxObj : ((TimeExpressions) obj).getTimex3()) {
            ...
            if (keepTimeTypeProvidedByNaf) {
                URI dynamicTypeUri = ValueFactoryImpl.getInstance()
                        .createURI(NWR.NAMESPACE, "timex3_" + tmxTypeUC.toLowerCase());
...
//                m.add(NWR.ANCHOR_TIME, tmxObj.getAnchorTimeID());
            ...
            String charS2 =
                    m.getUnique(NIF.BEGIN_INDEX, Integer.class) + "," + m.getUnique(NIF.END_INDEX, Integer.class);
            vars.entityMentions.put(charS2, m);
        }
    }
    ...
    private static void getTLinksMentions(TemporalRelations temporalRelations, processNAFVariables vars)
        ...
        for (Tlink fvObj : temporalRelations.getTlink()) {
            ...
            if (fvObj.getFrom().equalsIgnoreCase("tmx0")) {
                ...
                int returned = addTlinkRelTypeToMention(NWR.TLINK_FROM_TMX0,
                        vars.tLinkTypeMapper.get(fvObj.getRelType().toUpperCase()), mtest2, vars);
                if (returned == 1) {
            ...
            if (fvObj.getTo().equalsIgnoreCase("tmx0")) {
                ...
                int returned = addTlinkRelTypeToMention(NWR.TLINK_TO_TMX0,
                        vars.tLinkTypeMapper.get(fvObj.getRelType().toUpperCase()), mtest1, vars);
                if (returned == 1) {
                    vars.tlinkMentionsEnriched++;
                } else {
    ...
    private static int addTlinkRelTypeToMention(URI key, URI value,
        ...
        String charS = mention.getUnique(NIF.BEGIN_INDEX, Integer.class) + "," + mention
                .getUnique(NIF.END_INDEX, Integer.class);
	    ...
	    */
            ...
            for (PredicateAnchor tprd : prds) {
                ...
                if (tprd.getAnchorTime() != null) {
                    LinkedList<Wf> wfL = reorderWFAscending(
                            fromSpanGetAllMentionsTmx(((Timex3) tprd.getAnchorTime()).getSpan().getTarget(), vars),
                            vars);
                    if (wfL.size() > 0) {
                        Record mtest1 = Record.create();
                ...
                if (tprd.getBeginPoint() != null) {
                    LinkedList<Wf> wfL = reorderWFAscending(
                            fromSpanGetAllMentionsTmx(((Timex3) tprd.getBeginPoint()).getSpan().getTarget(), vars),
                            vars);
                    if (wfL.size() > 0) {
                        Record mtest1 = Record.create();
                        generateMIDAndSetIdWF(wfL, mtest1, vars);
                ...
                if (tprd.getEndPoint() != null) {
                    LinkedList<Wf> wfL = reorderWFAscending(
                            fromSpanGetAllMentionsTmx(((Timex3) tprd.getEndPoint()).getSpan().getTarget(), vars), vars);
                    if (wfL.size() > 0) {
                        Record mtest1 = Record.create();
                        generateMIDAndSetIdWF(wfL, mtest1, vars);
            ...
            deg += "| ANCHOR_TIME:" + mtmp.get(NWR.ANCHOR_TIME) + " | BEGIN_POINT:" + mtmp.get(NWR.BEGIN_POINT)
                    + " | END_POINT:" + mtmp.get(NWR.END_POINT);
	    /* 
		...
		*/
                if (prdGObj instanceof ExternalReferences) {
                    ...
                    for (ExternalRef exrObj : ((ExternalReferences) prdGObj).getExternalRef()) {
                        if (mtmp != null) {
                            ...
                            if (resourceValue != null) {
                                ...
                                // resourceValue == null)
                                logDebug(
                                        "xpath(//NAF/srl/predicate/externalReferences/externalRef@Resource(NULL)): predicateID("
                                                + predicateID + ")", vars);
                            }
                        } else {
                            logDebug(
                                    "Mapping error - Mention null - xpath(NAF/srl/predicate/externalReferences/externalRef): predicateID("
                                            + predicateID + ")", vars);
                        }
                    }
                    if (!eventTypeFound) {
                    ...
                    if (eventTypeFound) {
                        ...
                        String charS2 = mtmp.getUnique(NIF.BEGIN_INDEX, Integer.class) + "," + mtmp
                                .getUnique(NIF.END_INDEX, Integer.class);
                    ...
                    if (prdGObj instanceof Role) {
                        ...
                        if ((semRole != null) && (semRole.equalsIgnoreCase("AM-TMP"))) {
                            ...
                            if (checkAlreadyAcceptedMention(roleCharSpan, vars)) {
                                ...
                                logDebug(
                                        " ROL1: <srl> <role> found already existent mention for |" + roleCharSpan + "|",
                                        vars);
                            }
                        }
			...
			*/
			    ...
			    */
                            ...
                            charS = relationM.getUnique(NIF.BEGIN_INDEX, Integer.class) + "," + relationM
                                    .getUnique(NIF.END_INDEX, Integer.class);
                            if (createParticipationMentionFlag) {
                                logDebug("  ROL1: <srl> <predicate> <role> adding new participation mention for |"
                                        + charS + "|", vars);
                            } else {
                                logDebug(
                                        "  ROL1: <srl> <predicate> <role> adding new TLINK mention for |" + charS + "|",
                                        vars);
                            }
                            ...
                            for (ExternalReferences roleGOBJ : ((Role) prdGObj).getExternalReferences()) {
                                for (ExternalRef rexRefObj : roleGOBJ.getExternalRef()) {
                                    ...
                                    if (resourceValue != null) {
                                        ...
                                        URI resourceMappedValue = vars.srlExternalRefResourceTypeMapper
                                                .get(resourceValue);
                                        URIImpl valueURI;
                                        if (resourceMappedValue != null) {
                                            valueURI = getUriForSrlExternalRefResource(resourceValue, referenceValue);
                                        ...
                                        // resourceValue == null)
                                        logDebug(
                                                "xpath(//NAF/srl/predicate/role/externalReferences/externalRef@Resource(NULL)): RoleID("
                                                        + ((Role) prdGObj).getId() + ")", vars);
                                    }
                                }
            ...
            if (addMentionsFlag) {
                for (Span corefSpan : corefObj.getSpan()) {
                    ...
                    if (checkAlreadyAcceptedMention(corefCharSpan, vars)) {
                        logDebug("ROL1: <coref> id " + corefObj.getId()
                                + ": skipping already existent mention with charSpan |"
                                + corefCharSpan + "|", vars);
                        continue;
                    }
                    deg = "";
    ...
    private static Integer addOrMergeAMention(Record m, processNAFVariables vars) {
		...
		*/
                if (checkMentionReplaceability(vars.mentionListHash.get(charS), m)) {
                    ...
                    logDebug("Replacement with Mention: " + m.getID() + ", class(" + getTypeasString(m.get(RDF.TYPE))
                            + ")", vars);
                    return 0;
                }
                ...
                if (types.contains(NWR.PARTICIPATION.stringValue())) {
                    logDebug("Participation collision error, mentionID(" + m.getID() + ") class1(" + getTypeasString(
                            m.get(RDF.TYPE)) + "), class-pre-xtracted(" + getTypeasString(
                            vars.mentionListHash.get(charS).get(RDF.TYPE)) + ")", vars);
                } else {
                    logDebug("Generic collision error, mentionID(" + m.getID() + ") class1(" + getTypeasString(
                            m.get(RDF.TYPE)) + "), class-pre-xtracted(" + getTypeasString(
                            vars.mentionListHash.get(charS).get(RDF.TYPE)) + ")", vars);
                }
                return -1;
		...
		*/
                ...
                logDebug("Mention enrichment: " + m.getID() + ", class(" + getTypeasString(m.get(RDF.TYPE)) + ")",
                        vars);
                return 0;
            }
	...
	*/
        ...
        boolean isGenericOldM = (typesOld.contains(NWR.OBJECT_MENTION) && (oldM.get(NWR.ENTITY_TYPE) == null
                || oldM.get(NWR.ENTITY_TYPE).size() == 0));
        boolean isSpecificNewM = ((typesNew.contains(NWR.OBJECT_MENTION) && (newM.get(NWR.ENTITY_TYPE) != null
                && oldM.get(NWR.ENTITY_TYPE).size() > 0))
	...
	*/
    ...
    private static void generateTheMIdAndSetID_forParticipationMention(LinkedList<Term> eventTermList,
        ...
        int charEndOfEvent = Integer.parseInt(eventWordList.getLast().getOffset()) + Integer
                .parseInt(eventWordList.getLast().getLength());
        int charStartOfRole = Integer.parseInt(roleWordList.getFirst().getOffset());
        int charEndOfRole = Integer.parseInt(roleWordList.getLast().getOffset()) + Integer
                .parseInt(roleWordList.getLast().getLength());
    ...
    private static String getExtentOfParticipationMention(LinkedList<Term> eventTermList,
        ...
        for (Wf w : firstWL) {
            if (!mergedWordList.contains(w)) {
                mergedWordList.add(w);
            }
        }
        for (Wf w : secondWL) {
            if (!mergedWordList.contains(w)) {
